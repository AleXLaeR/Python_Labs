...
def draw_scaled_boxes(frame, input_img_dims, v_boxes, v_labels, v_scores):
    for box, label, score in zip(v_boxes, v_labels, v_scores):
        xmin = int(box.xmin * frame.shape[1] / input_img_dims[0])
        ymin = int(box.ymin * frame.shape[0] / input_img_dims[1])
        xmax = int(box.xmax * frame.shape[1] / input_img_dims[0])
        ymax = int(box.ymax * frame.shape[0] / input_img_dims[1])
...
def draw_boxes(image, boxes, labels, obj_thresh):
    for box in boxes:
        ...
        for i in range(len(labels)):
            print(box.classes[i], obj_thresh)
