...
class Cell(CanvasFigure):
    ...
    def _draw_upper_border(x: int, y: int, screen: pygame.Surface) -> None:
        ...
        pygame.draw.line(screen, BLACK, start_coords, end_coords, 2)
...
def draw_path(canvas: pygame.Surface, path_points: list[tuple[int, int]], step: int) -> None:
    if step > 1:
        pygame.draw.lines(canvas, GREEN, False, path_points[:step], 3)
...
def main():
    ...
    start_point = maze.grid[0][0]
    end_point = maze.grid[ROWS - 1][COLS - 1]
    ...
    while is_game_running:
        ...
        if step < len(path_points):
            step += 1

        draw_path(canvas, path_points, step)
